...
let
  ...
  macOnly = lib.optionalString pkgs.stdenv.isDarwin ''
    ...
    # 判定材料はコマンドラインの引数と、ssh -G が解決した設定 (hostname 等) の両方。
    #
    # Why not (引数の見た目だけで判定しない理由): ~/.ssh/config に Host エイリアスを
    # 書いて `ssh wsl` と打つようにした途端、コマンドラインから IP が消えて判定が外れ、
    # タイトルが "ssh wsl" のまま残った (Ghostty 側が実行中コマンドを出したもの)。
    # ssh -G は接続せずに設定を解決して出力するだけの副作用の無いオプションなので、
    # エイリアス経由でも解決後の hostname で拾える。エイリアス名そのものも見ているのは、
    # -G が失敗する呼び方 (引数不足など) をしたときの保険。
    _window_title_for_ssh() {
      local target="$* $(command ssh -G "$@" 2>/dev/null)"
      case "$target" in
        # WSL の入った Windows 機 (ssh wsl / ssh m1205062@192.168.1.254 -p 2222)。
        # ユーザ名やオプションの並び順に依存しないよう、ホストだけで判定する。
        *192.168.1.254*|(|*[[:space:]])wsl(|[[:space:]]*)) print -r -- $'\U000F05B3 Win' ;;
      esac
    }
